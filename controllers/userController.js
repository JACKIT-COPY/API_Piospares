...
const userSchema = Joi.object({
  ...
  role: Joi.string().valid('SuperManager', 'Manager', 'Cashier', 'Accountant', 'Developer', 'Shareholder').optional(),
...
const inviteUser = async (req, res) => {
  ...
  try {
    ...
    const globalRole = ['Accountant', 'Developer'].includes(role);
    // Enforce single branch for all non-SuperManager and non-global roles
    if (!globalRole && role !== 'SuperManager' && branchIds?.length > 1) {
      return res.status(400).json({ message: `${role}s can only be assigned to a single branch` });
    }
    if (!globalRole && (!branchIds || branchIds.length === 0)) {
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (updateData.branchIds) {
      ...
      const globalRole = ['Accountant', 'Developer'].includes(roleToCheck);
      if (!globalRole && roleToCheck !== 'SuperManager' && updateData.branchIds.length > 1) {
        return res.status(400).json({ message: `${roleToCheck}s can only be assigned to a single branch` });
      }
      if (!globalRole && updateData.branchIds.length === 0) {
        return res.status(400).json({ message: 'At least one branch must be assigned' });
      }
    }
